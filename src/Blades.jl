...
function quadmetric_expr(ns, namea, nameb, abmetric)
    Expr(:(=), 
         Expr(:call, 
              Expr(:., :Blades, Expr(:quote, #QuoteNode 
                                     :quadmetric, 
                                    )), 
...
macro generate_basis(metric, export_types=false, generate_reciprocals=false, zero_mixed_index=false)
  ...
  push!(dual_fcn,
        Expr(:(=), 
             Expr(:call, Expr(:., :Blades, Expr(:quote, #QuoteNode
                                                      :dual
                                                     )), Expr(:(::), :k, I₀)), 
              Expr(:block,
                        Expr(:., :k, 
  ...
  pseudoscalar_fcn = map(e) do eᵢ
    eᵢname = eᵢ.name
    [:(Blades.pseudoscalar(::Type{K}) where {K<:$eᵢname} = $I₀),
     :(Blades.pseudoscalar(::Type{K}) where {T,K<:$eᵢname{T}} = $I₀{T})]
  end |> flatten
  ...
  # zpseudoscalar_fcn = :(Blades.pseudoscalar(::Type{K}) where {K<:ZForm} = $I₀)
